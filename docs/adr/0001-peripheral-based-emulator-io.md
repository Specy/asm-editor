...
- The initial implementation creates peripherals inside the Emulator (no injection API). [ADR 0004](./0004-inject-screens-at-emulator-boundary.md) extends this design with caller-supplied Screen instances at the Emulator boundary; implementation is pending. Terminal customization through an interactive input source remains a separate seam.
