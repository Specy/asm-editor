...
| M8  | A program drawing two pixels              | Step forward until both pixels are on the Screen, then Undo: the Screen walks back with the code                                                                                                 | ADR 0005           | Pass, phase 6, under node (`M68KEmulator.test.ts`): ink 2 → 1 → 0 over the undos. Not walked by hand in the GUI                                           |
...
| M10 | Any of the four                           | Build and Clear execution each leave a blank Screen, and the keyboard and mouse state is forgotten with it                                                                                       | Design record      | Pass, phase 6: blank after Build and after Stop, which is this editor's Clear execution. Project close not run                                            |
