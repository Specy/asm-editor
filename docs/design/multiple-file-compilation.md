...
Implementation update on 2026-09-11: the application now uses the common Files/Entry model throughout and supports multi-file assembly for every assembly Target. The x86 package accepts a virtual Project and lets NASM resolve `%include`/`incbin` from the staged text and binary Files. Update on 2026-09-12: it also assembles each assembly source that nothing includes as its own translation unit and links them together, so `global` in one File resolves an `extern` in another; a File another source includes is assembled only as part of that source, which `nasm -M` is asked about rather than guessed at. A failed link now reports `ld`'s diagnostics against the File and line they name, because a build that returned success and left nothing to run made the next step fail with a message about the emulator instead of the mistake. MIPS and RISC-V expose runtime file operations. The editor uses persistent per-file models and a custom overlay sidebar; source locations, diagnostics, highlights, generated instructions, history, stack frames, and navigation retain File identity. Project archives, exact byte storage, the 16 MiB/4,096-File live limits, testcase isolation, Debug-session locking, and coordinated file-operation Undo are implemented as accepted below.
