...
| x86                      | The adapter currently calls the core's single-string `compile` and `checkCode` methods. Multi-file core support has not been established.                                                                                                                                                                                                                       |
...
| x86                             | The public compiler method still accepts one source string.                         | The wrapper exposes its internal Emscripten FS but has no MIPS/RISC-V-style file callbacks; sharing reactive Project Files and restoring file operations on Undo requires a separate integration. |

Inject the shared FileSystem into every Emulator and implement multi-file assembly for M68K, MIPS, RISC-V, and Z80. Wire program-level file I/O for MIPS/RISC-V in this change. Defer new M68K file traps, the Z80 filesystem port protocol, and the x86 runtime bridge; x86's adapter consumes its Entry file through the common file model while retaining single-source Core compilation.
...
Implementation note on 2026-09-09: this project pins `@specy/s68k` 2.1.0 and both `@specy/mips` and `@specy/risc-v` 3.0.0. The published RISC-V 3.0.0 generated bundle declares `readFile` but omits its JavaScript bridge and read-syscall branch; both MARS/RARS-derived 3.0.0 bundles also keep non-undoable descriptor tables whose open, close, and completion cleanup conflict with the shared FileSystem's retained Debug session. Exact-version, anchor-checked, idempotent postinstall scripts patch those generated bundles locally: [`patch-riscv-file-read.mjs`](../../scripts/patch-riscv-file-read.mjs) and [`patch-emulator-file-lifetime.mjs`](../../scripts/patch-emulator-file-lifetime.mjs). Installation fails explicitly on another version or changed generated shape instead of applying an uncertain patch. These shims should be removed when matching upstream packages publish the required bridges and lifetime semantics. For s68k 2.1.0, the adapter keeps ordinary text paths for `include` and rewrites resolved `incbin` operands to private byte aliases so `incbin` embeds shared UTF-8/binary FileSystem bytes exactly without changing M68K character-literal semantics.
