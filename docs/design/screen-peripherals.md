...
| MIPS, RISC-V | MARS's five parameters with its defaults     | The user, from the Screen panel's header, or the program, through the `@screen` comment directive |

- The MIPS and RISC-V parameters are saved in the project data as a display field, because a project must reopen with them. Testcases run with the same configuration. Changing a parameter re-syncs the Screen from memory immediately, as MARS does.
- A MIPS or RISC-V program states the display it wants in a **`@screen` comment directive** (`# @screen unit=1 width=256 height=256 base=display`), parsed at compile time so the Screen is configured before the first instruction and during a Testcase run. `base` takes a label, resolved through the assembler, so a program never has to know the address. Anything MARS could not be configured with is a compile-time warning on the directive's line and never an error, because a comment must not stop a program that assembles in MARS and RARS; a program with no directive changes nothing, and a hand edit in the popover wins until the next Build. Added on 2026-09-06, replacing the row above's "the user, from the Screen panel's header" as the only way in: every example used to state the parameters in prose for the user to copy by hand.
- Zoom is GUI only: the Screen fits its panel exactly, filling whichever dimension constrains it, the MARS unit size is what the panel's actual-size position means, and zoom is not persisted per project. Integer scaling was the rule until 2026-09-06, when it was dropped: it left a small Screen at 1:1 in a panel several times its size, and the fraction it discarded was also what pushed the image into a scrollbar.
...
- Placement: a Screen panel in the project page's right column next to memory, visible by default for the four environments and collapsible through a setting like the memory toggle. The interactive editor shows it as a panel in the fullscreen layout and behind a toggle in the small layout, so lectures, exams and embeds get it. The transcript comes **before** the Screen, which is therefore last in the column — reversed on 2026-09-06 at the user's request, where this row used to say the transcript stays at the bottom. Focus indication and context-menu suppression belong to the panel. The panel fits the Screen to its box by default, at the exact scale the constraining dimension allows rather than a whole number of screen pixels, and its header offers a floating window that opens at the top right of the viewport, leaving the execution controls visible, and is then **dragged wherever the user wants** — it is the app's own draggable window, whose header carries the panel's title and controls and whose right-hand button puts the Screen back in the page. Dragging was added on 2026-09-06 at the user's request, where this row used to say the window was "anchored to the right of the viewport". The window floats over the page, not over the application: the Settings, Documentation and Share drawers, a program's input prompt and the toasts all open in front of it, because it has no backdrop and is meant to be watched while the page underneath is used. Added on 2026-09-06 by the review, which found the drawers opening behind it.
...
Under the five percent the ADR budgets, on every Core. One yield costs about 1.1 ms under node, which falls back to a timer, so what keeps the cost down is the length of the slice rather than the price of the yield. In a browser a yield is a posted message and costs 0.8 ms; it used to be `scheduler.yield()` at 0.2 to 0.5 ms, which was cheaper and starved the browser of rendering, and the frames that cost are in the [rendering performance record](./screen-rendering-performance-research.md#the-yield-between-two-slices-was-starving-the-browser).
...
| `SCREEN_ACTIVITY_MS`               | —      | 200    | Added 2026-09-06. How long after the last visible change a program still counts as animating: the dirty flag alone gave the very next slice the compute budget, and a browser measurement put the M68K bouncing ball at 27.5 delivered frames a second that way against 40.6 with the window |
...
### What the browser said, later

The measurements above are producer-side and were taken under node: the harness counts a frame when `markPainted` is called on a timer, and no canvas is involved. A browser pass on 2026-09-06 measured the frames that actually reach the canvas, and found three things this section could not have: canvas submission is under one percent of the wall clock, so dirty-rectangle uploads and an opaque context are both not worth having; `scheduler.yield()` held the browser to about nine frames a second for the whole of any run; and the panels an adapter refreshed on every Core interrupt cost more than the drawing did. The evidence, the changes it justified and the ones it refuted are in the [rendering performance record](./screen-rendering-performance-research.md#measured-and-what-was-implemented).
