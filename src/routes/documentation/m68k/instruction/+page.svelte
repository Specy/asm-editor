<script lang="ts">
    ...
    import { type InstructionDocumentation, M68KUncompoundedInstructions } from '$lib/languages/M68K-documentation'
    ...
    const instructions = Array.from(M68KUncompoundedInstructions.values())
    const groups = new Array(26).fill(0).map((_, i) => {
        const letter = String.fromCharCode(65 + i)
        return {
            letter,
            instructions: instructions.filter(e => e.name[0].toUpperCase() === letter)
        } satisfies Group
    })
</script>
