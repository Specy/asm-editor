...
holds the `10010000` it was given before the loop. Their difference is 15, which is the length, and
that is what C's `strlen` compiles to. The M68K version of the same loop subtracts one more at the
end, because its `tst.b (a0)+` has already stepped past the byte it tested; here the load and the
step are two instructions and the branch happens between them.
...
A second string sits right behind the first one in memory. Try changing `text: .asciiz` to
