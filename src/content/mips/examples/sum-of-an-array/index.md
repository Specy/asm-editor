...
mode steps the register by the size of what it read; MIPS has one addressing mode for a load and it
is `offset(base)`, so the step is an instruction of its own and the 4 in it is yours.
