...
The bytes come out backwards from how you would read the number, because RISC-V is little endian and
...
instructions every time it runs, an `auipc` that works the address out and the `lw` itself, both
into `t1`. MIPS spends its `$at` on the same line and RISC-V has no such register, which is also why
a store to a label is written `sw t1, total, t2`, with a third operand naming the register the
address is built in.
