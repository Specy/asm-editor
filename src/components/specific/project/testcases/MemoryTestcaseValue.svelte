	...
	{:else}
		{#if value.type === 'number'}
			<div  style="word-break: break-all;">
				At address <b>${toHexString(value.address)}</b>, expect <b>${toHexString(value.expected, value.bytes * 2)}</b> (of <b>{value.bytes}</b> bytes)
			</div>
		{/if}
		...
		{#if value.type === 'number-chunk'}
			<div  style="word-break: break-all;">
				At address <b>${toHexString(value.address)}</b>, expect <b>[{
				value.expected.map((v) => `0x${toHexString(v, 2)}`).join(", ")
			}]</b>
			</div>
		{/if}
	{/if}
