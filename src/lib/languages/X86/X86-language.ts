...
import {
    ...
    X86Directives,
    X86SizeSpecifiers,
} from './X86-grammar'; // Adjust the import path as needed
...
export function createX86CompletitionProvider(monacoInstance: typeof monaco): monaco.languages.CompletionItemProvider {
    return {
        ...
        provideCompletionItems: (model, position): monaco.languages.ProviderResult<monaco.languages.CompletionList> => {
            ...
            X86Directives.forEach(dir => {
                ...
                if (dir.toLowerCase().startsWith(word) || (word.startsWith('%') && dir.startsWith('%') && dir.toLowerCase().startsWith(word))) {
                    suggestions.push(createCompletionItem(
                        monacoInstance,
                        dir,
...
export function createX86HoverProvider(monacoInstance: typeof monaco): monaco.languages.HoverProvider {
    return {
