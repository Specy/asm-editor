...
export async function getAllCourses() {
    ...
    return Object.entries(content).map(([path, course]) => {
...
export async function getCourse(slug: string) {
    ...
    const lectures = Object.entries(await import.meta.glob(`$content/*/*/*/meta.json`, { eager: true }))
        .filter(([path]) => {
    ...
    const modules = Object.entries(await import.meta.glob(`$content/*/*/meta.json`, { eager: true }))
        .filter(([path]) => {
            return path.startsWith('/src/content/' + course.slug)
        })
        .map(([path, lecture]) => {
...
export async function getLectureContent(courseSlug: string, moduleSlug: string, lectureSlug: string) {
    return await fs.readFile(`src/content/${courseSlug}/${moduleSlug}/${lectureSlug}/index.md`, 'utf-8')
}
